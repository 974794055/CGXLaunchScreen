...
@implementation CGXLaunchScreenCache
...
+(BOOL)saveImageData:(NSData *)data imageURL:(NSURL *)url{
    NSString *path = [NSString stringWithFormat:@"%@/%@",[self CGXLaunchScreenCachePath],[self keyWithURL:url]];
    if (data) {
        BOOL result = [[NSFileManager defaultManager] createFileAtPath:path contents:data attributes:nil];
        if (!result) CGXLaunchScreenLog(@"cache file error for URL: %@", url);
        return result;
    }
    return NO;
}
...
+(BOOL)saveVideoAtLocation:(NSURL *)location URL:(NSURL *)url{
    NSString *savePath = [[self CGXLaunchScreenCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
...
+ (NSString *)CGXLaunchScreenCachePath{
    NSString *path =[NSHomeDirectory() stringByAppendingPathComponent:@"Library/CGXLaunchScreenCache"];
...
+(NSString *)imagePathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self CGXLaunchScreenCachePath] stringByAppendingPathComponent:[self keyWithURL:url]];
}

+(NSString *)videoPathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self CGXLaunchScreenCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
}

+(NSString *)videoPathWithFileName:(NSString *)videoFileName{
    if(videoFileName.length==0) return nil;
    return [[self CGXLaunchScreenCachePath] stringByAppendingPathComponent:[self videoNameWithURL:[NSURL URLWithString:videoFileName]]];
}
...
+(void)clearDiskCache{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *path = [self CGXLaunchScreenCachePath];
        [fileManager removeItemAtPath:path error:nil];
        [self checkDirectory:[self CGXLaunchScreenCachePath]];
    });
}
...
+(void)clearDiskCacheExceptImageUrlArray:(NSArray<NSURL *> *)exceptImageUrlArray{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        CGXLaunchScreenLog(@"allFilePath = %@",allFilePaths);
    });
}
...
+(float)diskCacheSize{
    NSString *directoryPath = [self CGXLaunchScreenCachePath];
    BOOL isDir = NO;
...
+ (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        CGXLaunchScreenLog(@"create cache directory failed, error = %@", error);
    } else {
        [self addDoNotBackupAttribute:path];
    }
    CGXLaunchScreenLog(@"CGXLaunchScreenCachePath = %@",path);
}
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        CGXLaunchScreenLog(@"error to set do not backup attribute, error = %@", error);
    }
}
