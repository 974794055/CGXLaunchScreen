...
@protocol CGXLaunchAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(CGXLaunchAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface CGXLaunchAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<CGXLaunchAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation CGXLaunchAnimatedImageView
...
- (void)setAnimatedImage:(CGXLaunchAnimatedImage *)animatedImage
{
...
{
    // Presision is set to half of the `kCGXLaunchAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kCGXLaunchAnimatedImageDelayTimeIntervalMinimum;
