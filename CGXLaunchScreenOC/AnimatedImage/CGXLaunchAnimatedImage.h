...
extern const NSTimeInterval kCGXLaunchAnimatedImageDelayTimeIntervalMinimum;

//
//  An `CGXLaunchAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `CGXLaunchAnimatedImageView`.
...
//
@interface CGXLaunchAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `CGXLaunchAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `CGXLaunchAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface CGXLaunchAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [CGXLaunchAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
