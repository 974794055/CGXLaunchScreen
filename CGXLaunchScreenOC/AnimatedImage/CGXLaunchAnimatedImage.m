...
const NSTimeInterval kCGXLaunchAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `CGXLaunchAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, CGXLaunchAnimatedImageDataSizeCategory) {
    CGXLaunchAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    CGXLaunchAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    CGXLaunchAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    CGXLaunchAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, CGXLaunchAnimatedImageFrameCacheSize) {
    CGXLaunchAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    CGXLaunchAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    CGXLaunchAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    CGXLaunchAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol CGXLaunchAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(CGXLaunchAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(CGXLaunchAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(CGXLaunchAnimatedImage *)animatedImage;
@end
#endif


@interface CGXLaunchAnimatedImage ()
...
@property (nonatomic, strong, readonly) CGXLaunchAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<CGXLaunchAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation CGXLaunchAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > CGXLaunchAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > CGXLaunchAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [CGXLaunchAnimatedImage class]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            // Note that CGXLaunchAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
...
- (instancetype)init
{
    CGXLaunchAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `CGXLaunchAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (self) {
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        // Support frame delays as low as `kCGXLaunchAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kCGXLaunchAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kCGXLaunchAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= CGXLaunchAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= CGXLaunchAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = CGXLaunchAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = CGXLaunchAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
...
{
    CGXLaunchAnimatedImage *animatedImage = [[CGXLaunchAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    CGXLaunchAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[CGXLaunchAnimatedImage class]]) {
        CGXLaunchAnimatedImage *animatedImage = (CGXLaunchAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `CGXLaunchAnimatedImage`: %@", image);
    }
...
{
    self.frameCacheSizeMaxInternal = CGXLaunchAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(CGXLaunchAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)CGXLaunchAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = CGXLaunchAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(CGXLaunchAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
@implementation CGXLaunchAnimatedImage (Logging)
